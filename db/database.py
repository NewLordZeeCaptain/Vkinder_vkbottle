...
def reg_user(vk_id: int, first_name: str, last_name: str, sex: int, city: int):
    user = User.create(
        vk_id=vk_id, first_name=first_name, last_name=last_name, sex=sex, city=city
    )
    return user
...
def add_to_fav(user_id: int, vk_id: int):
    fav = Favorite.create(vk_id=vk_id).where(User.vk_id == user_id)
    return fav


def add_to_black(user_id: int, vk_id: int):
    black = Blacklist.create(vk_id=vk_id).where(User.vk_id == user_id)
    return black
